...
function generateOrderID() {
    try {
        ...
        if (newOId < 10) {
            $("#idText").val("O00" + newOId);
        } else if (newOId < 100) {
            $("#idText").val("O0" + newOId);
        } else {
            $("#idText").val("O" + newOId);
        }
    } catch (e) {
...
$("#cmbCustIDs").click(function () {
    ...
    if(cus!=null){
        ...
        $('#txtPCustNumber').val(cus.getCustomerNumber());
    }
});
...
$("#btn-addToCart").click(function () {
    ...
    if($('#qtyYouWant').val()!==""){
        if(qty<Oqty){
            ...
            loadCart();
            getTotal();
            $("#itemName,#price,#qtyOnHand,#qtyYouWant").val("")
        }
    }else{
...
$("#btn-purchase-order").click(function () {
    ...
    $('#addToCartTable').empty();
    $("#itemName,#price,#qtyOnHand,#qtyYouWant,#name,#address").val("")
});
$("#cash").on('keyup', function (eventOb) {
